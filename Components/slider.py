...
df = pd.read_csv("Data/severe-storms.csv")
...
app.layout = html.Div(
    [
        ...
        dcc.RangeSlider(
            min=df["Severe Storm Costs (Billions)"].min(),
...
def update_graph(n_storms, dollar_range):
    ...
    bool_series2 = df["Severe Storm Costs (Billions)"].between(
        dollar_range[0], dollar_range[1], inclusive="both"
    )
    filtered_year = df[bool_series2]["Year"].values
    fig["data"][0]["marker"]["color"] = [
        "orange" if c in filtered_year else "blue" for c in fig["data"][0]["x"]
    ]
