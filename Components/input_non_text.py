...
df.rename(
    columns={"Data Item": "Affected by", "Value": "Percent of Colonies Impacted"},
    inplace=True,
)
        ...
        html.Div(
            [
                dcc.Input(
                    id=f"my_{x}",
                    type=x,
                    placeholder=f"insert {x}",  # A hint to the user of what can be entered in the control
                    debounce=True,  # Changes to input are sent to Dash server only on enter or losing focus
                    min=2015,
                    max=2019,
                    step=1,  # Ranges of numeric value. Step refers to increments
                    minLength=0,
...
@app.callback(
    ...
    [Input(component_id=f"my_{x}", component_property="value") for x in input_types],
)
def update_graph(
    num_year,
    pwd_state,
