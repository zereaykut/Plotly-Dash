...
df = pd.read_csv("Data/suicide-rate-1990-2017.csv")
...
fig = go.Figure(
    ...
    layout=go.Layout(
        xaxis=dict(
            range=[0, 60],
            autorange=False,
            title=dict(text="suicide rate (deaths per 100,000)", font=dict(size=18)),
        ),
    ...
    frames=[
        go.Frame(
            ...
            layout=go.Layout(
                ...
                yaxis=dict(range=[-0.5, 5.5], autorange=False, tickfont=dict(size=14)),
                title=dict(
                    text="Suicide Rates per Country: " + str(value["year"].values[0]),
                    font=dict(size=28),
                ),
            ),
        )
