...
app.layout = html.Div(
    [
        ...
        html.Div(
            [
                dcc.Dropdown(
                    ...
                    value="gapminder",
                    options=[
                        {"label": "Country Data", "value": "gapminder"},
        ...
        dcc.Store(
            id="store-data", data=[], storage_type="memory"
        ),  # "local" or "session"
    ]
)


@callback(Output("store-data", "data"), Input("data-set-chosen", "value"))
...
@callback(Output("table-placeholder", "children"), Input("store-data", "data"))
def create_graph1(data):
    dff = pd.DataFrame(data)
    # 2. convert string like JSON to pandas dataframe
    # dff = pd.read_json(data, orient="split")
