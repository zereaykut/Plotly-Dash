...
app.layout = html.Div(
    [
        dcc.Interval(
            ...
            interval=1
            * 3000,  # increment the counter n_intervals every interval milliseconds
...
@app.callback(
    [Output("output_data", "children"), Output("mybarchart", "figure")],
...
def update_graph(num):
    ...
    if num == 0:
        ...
        y_data = num
        fig = go.Figure(
            data=[go.Bar(x=[1, 2, 3, 4, 5, 6, 7, 8, 9], y=[y_data] * 9)],
...
@app.callback(Output("my_interval", "max_intervals"), [Input("input_text", "value")])
